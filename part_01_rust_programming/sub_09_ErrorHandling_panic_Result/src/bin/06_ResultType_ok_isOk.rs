// This script demo the methods ``.is_ok()`` and ``.ok()`` of ``Result<T, E>``
...
fn main() {
    ...
    println!("successful Results = {:?}", successful_results); // [Ok(10), Ok(20), Ok(30)]

    // ok() is especially useful with filter_map().
    // Each Result becomes Option<T>:
    //
    // Ok(value) -> Some(value), so the value is kept
    ...
    println!("successful values = {:?}", successful_values); // [10, 20, 30]
}
